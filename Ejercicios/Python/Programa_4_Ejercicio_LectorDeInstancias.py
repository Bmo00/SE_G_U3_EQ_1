...
qtCreatorFile = "Programa_4_Ejercicio_LectorDeInstancias.ui"  # Nombre del archivo aquí.
...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def Crear(self):
        ...
        if not nombre=="" and not totElementos=="" and not train=="": #evalua que los line edits no esten vacios
            ...
            train=int(train)
            Datos = pd.read_csv(f"../Archivos/{nombre}.csv", sep=',') #almacena en Datos el csv con ayuda de pandas
            if train>totElementos or totElementos>len(Datos): # evalua que no exceda el total de elementos dados
                ...
                with open('../Archivos/Ejercicio_4_Training.csv', 'w', newline='') as file:
                    mywriter = csv.writer(file, delimiter=',')
                    mywriter.writerows(MTrain)

                with open('../Archivos/Ejercicio_4_Test.csv', 'w', newline='') as file:
                    mywriter = csv.writer(file, delimiter=',')
