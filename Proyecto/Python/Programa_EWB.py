...
print("\nPROCEDIENDO CON LA DISCRETIZACION...")

Mds=[];PM=[];SM=[];LA=[];Mat=[]
CAtributos=5
Dic={}

Datos = pd.read_csv("XPokemones_TrainingConEWB.csv", sep=',')

with open("YResultadoTest.csv", "r") as Test:
    MReal2=Test.read().replace("\n","")
MReal2=MReal2.split(",")
MReal2 = np.array(MReal2).reshape(1,CAtributos + 1)
print(MReal2)
...
def EWB(diccionario,K,generar): #Se manda como parametro el diccionario, el total de k y si se quiere o no
    ...
    for i in diccionario:
        ...
        for j in range(K):
            ...
            for f2 in range(CAtributos):
                if int(MReal2[0][c])>=L1 and int(MReal2[0][c])<=L2:
                    MReal2[0][c]=int(f"{j+1}")
                    break
                    #Esto lo que hace es comparar si en la primera iteracion que datos son V1, es decir
        ...
        c += 1
    print("\n",MReal,"\n")
    print("VALORES DISCRETIZADOS: ",MReal2)

    if(generar==True): #Generar el dataset pero primero evaluara que se le haya dicho que si
        with open('ZPTraining_Discretizado.csv', 'w', newline='') as file:
            mywriter = csv.writer(file, delimiter=',')
            mywriter.writerows(MReal)

        with open('ZPTest_DiscretizadoSE.csv', 'w') as file:
            mywriter = csv.writer(file, delimiter=',')
            mywriter.writerows(MReal2)

        with open('ZPTest_DiscretizadoCE.csv', 'w', newline='') as file:
            mywriter = csv.writer(file, delimiter=',')
            mywriter.writerows(" ")
            mywriter.writerows(MReal2)
...
EWB(diccionario=Dic,K=7,generar=True) #Este es el que controlara ya saben se mandan los parametros por aqui
