...
####################################################################################################
#EJECUCIÓN PRINCIPAL
archivo = open("ZPTraining_Discretizado.csv")
...
casos = []
try:
    with open("ZPTest_DiscretizadoSE.csv","r") as archivo:
        contenido = archivo.readlines()
    ##############QUITA \n
    for i in range(len(contenido)):
        contenido[i] = contenido[i].replace("\n", "")
...
for k in range(len(casos)):  # Por cada caso
    ...
    temp = arbol
    existe = True
    while (temp.getEtiqueta() == "") and existe:
        i = 0
        while i < len(toEvaluar) and existe:
            if temp.getAtributo() == toEvaluar[i].nombreAtributo:
                existe = toEvaluar[i].etiqueta in temp.getRamas() ##comprueba que exista una rama ...
                if existe:
                    temp = temp.getRamas()[toEvaluar[i].etiqueta]
                else:
                    print("No existe informacion para evaluar este caso...{Seleccion Aleatoria}")
            i+=1
    claseReal = casos[k][len(casos[k]) - 1]
    print("Clase Asignada: " + temp.getEtiqueta() + " Clase Real: " + claseReal + " Evaluacion: " + str(
        claseReal == temp.getEtiqueta()));
    if claseReal == temp.getEtiqueta():
        totalCorrectos += 1

Res=str(round(totalCorrectos / len(casos) * 100.0, 4))
print("\nRendimiento : " + Res);
